# Eurobox Packing Lists

For a kit for about 20 builders at the same time. Print each list and put it in a sleeve on the front of the box. See [Eurobox system](../wiki/06-Eurobox-System.md).

Columns: **Target** = quantity that belongs in the box · **Out** = counted before the event · **Back** = counted after.

---

## 🔵 Blue 1: Cutting and shaping (600 × 400 × 220)

| Item | Target | Out | Back |
|---|---|---|---|
| Coping saws, with spare blades | 6 | | |
| Japanese pull saws, small | 2 | | |
| Mitre box / saw guide | 2 | | |
| Hand drills (manual) + drill bits 2–8 mm | 3 | | |
| Awls | 6 | | |
| Rasps / files | 4 | | |
| Sanding blocks | 6 | | |
| Carving knives with blunt tip (for older kids, only after briefing) | 4 | | |

## 🔵 Blue 2: Measuring, holding, assembling (600 × 400 × 220)

| Item | Target | Out | Back |
|---|---|---|---|
| Screw clamps / quick clamps | 10 | | |
| Folding rulers / tape measures | 4 | | |
| Rulers 30 cm | 6 | | |
| Try squares | 3 | | |
| Scissors (incl. left-handed) | 8 | | |
| Small hammers | 4 | | |
| Screwdrivers (set) | 2 | | |
| Pliers / side cutters | 3 | | |
| Pencils, carpenter's pencils | 20 | | |
| A4 paper templates (card) | 10 | | |

## 🟢 Green 1 + 2: Natural materials (600 × 400 × 320)

| Item | Target | Out | Back |
|---|---|---|---|
| Softwood offcuts, sorted by size | lots | | |
| Wooden slats, strips, dowels (various diameters) | lots | | |
| Wine corks (natural) | 300+ | | |
| Bamboo sticks, reed | 30 | | |
| Wooden skewers, toothpicks | 1 pack each | | |
| Wooden clothes pegs | 50 | | |
| Cotton, jute and hemp string | 5 rolls | | |
| Cotton and linen fabric scraps | 1 bag | | |
| Beeswax block | 2 | | |

## 🟡 Yellow 1 + 2: Reused materials (600 × 400 × 320)

| Item | Target | Out | Back |
|---|---|---|---|
| Plastic bottles with caps, rinsed, various sizes | 30 | | |
| Drink cartons, rinsed and dried | 30 | | |
| Plastic containers (yoghurt pots etc.), clean | 20 | | |
| Old T-shirt fabric | 1 bag | | |
| Tin lids with folded edges | 10 | | |

## ⚪ Grey 1: Connectors (400 × 300 × 120)

| Item | Target | Out | Back |
|---|---|---|---|
| Rubber bands (natural rubber), counted | 200 | | |
| Screws, assorted (in a divider box) | 1 box | | |
| Nails, small | 1 box | | |
| Wire, soft | 1 roll | | |
| Wooden dowels 6 and 8 mm | 50 each | | |

## ⚪ Grey 2: Consumables (400 × 300 × 120)

| Item | Target | Out | Back |
|---|---|---|---|
| Sandpaper (grits 80, 120, 180) | 10 each | | |
| Wood glue D3 (small bottles) | 3 | | |
| Water-based paints, brushes | 1 set | | |
| Cloakroom tickets (double, numbered, roll): start number + ice cream stub | 1 roll | | |
| Masking tape (for marking only, not for boats) | 2 | | |

## 🔴 Red: Safety (400 × 300 × 120)

| Item | Target | Out | Back |
|---|---|---|---|
| First aid kit (checked, not expired) | 1 | | |
| Extra plasters, tweezers, tick remover | 1 set | | |
| Safety glasses, child and adult sizes | 15 | | |
| Work gloves, child and adult sizes | 15 pairs | | |
| Sunscreen | 2 | | |
| Emergency sheet: numbers, address, map | 1 | | |
| Tool briefing picture cards | 1 set | | |

## ⚫ Black: Race office (600 × 400 × 220)

| Item | Target | Out | Back |
|---|---|---|---|
| Boat register (clusters of three), score sheets | 1 set | | |
| Builder pledges, photo consent forms (blank) | 50 each | | |
| Certificates (blank) | 60 | | |
| Clipboards, pens | 6 | | |
| Stopwatches (or phones) | 3 | | |
| Prize catalogue, sponsor sheets | 1 set | | |
| Kitchen scale (1 g, up to 5 kg) + ruler 50 cm | 1 each | | |
| A4 grid mat for the photo station | 1 | | |
| Gold talers (one per participant + 20%) | | | |
| Silver talers (one per expected spectator + 20%) | | | |
| Voting containers with number cards | 50 | | |
| Extra ice cream vouchers for children without their own boat | | | |
| Rubber ducks, numbered (optional) | | | |
| Prize bags | | | |
| Megaphone + batteries (charged) | 1 | | |
| "No photo" stickers / wristbands | 20 | | |
| Kit logbook | 1 | | |
| Cash box + receipt book (grown-up start fees, duck race) | 1 | | |
