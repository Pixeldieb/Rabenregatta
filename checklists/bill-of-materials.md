# Bill of Materials (BOM)

Everything you need for a Rabenregatta, in one list. Use it to plan, borrow, buy and ask sponsors.

**Sized for:** about **20 builders at the same time** at the building stations, **30–60 boats** and **100–300 spectators**. Scale up or down from there.

**Columns:**

- **Type:** **R** = reusable (buy or build once, use for years), **C** = consumable (refill every event)
- **Source:** where it usually comes from. **Borrow** = club, fire brigade, school, municipality · **Donate** = collected or given · **Build** = make it yourselves · **Buy**
- Box numbers refer to the [Eurobox system](../wiki/06-Eurobox-System.md). The detailed box contents with count columns are in the [packing lists](eurobox-packing-lists.md).

**Golden rule:** Borrow before you buy. Many villages already own tents, benches and tables. A complete kit can travel from village to village.

---

## 1. Tool boxes

Our core tool list comes from practice. Items marked ➕ are recommended additions.

### 🔵 Blue 1: Cutting (Eurobox 600 × 400 × 220)

| Item | Qty | Spec / note | Type | Source |
|---|---|---|---|---|
| Hand saws, small | 4 | Fine teeth for offcuts and slats | R | Buy / Borrow |
| Garden pruners | 4 | For branches, reed, bamboo | R | Borrow |
| Branch saws, folding | 2 | For thicker branches and driftwood | R | Borrow |
| Scissors | 8 | Including 2 left-handed | R | Buy |
| Cutters (snap-off blade) | 6 | On a craft mat, with cut-resistant glove | R | Buy |
| Spare cutter blades | 1 pack | | C | Buy |
| Knives | 4 | Older kids after briefing, or adults | R | Buy |
| Paper punches | 5 | Assorted shapes, for flags, sails and decoration. Collect the punched bits | R | Buy / Donate |
| Sanding sponges | 10 | Medium and fine | C | Buy |

### 🔵 Blue 2: Assembling and measuring (Eurobox 600 × 400 × 220)

| Item | Qty | Spec / note | Type | Source |
|---|---|---|---|---|
| Hammers, small | 4 | Nails are in Grey 1 | R | Buy / Borrow |
| Pliers | 3 | | R | Buy / Borrow |
| Side cutters | 2 | For wire | R | Buy / Borrow |
| ➕ Awls | 6 | Pre-drilling corks and cartons | R | Buy |
| ➕ Screw clamps / quick clamps | 10 | Hold the work, free both hands | R | Buy / Borrow |
| ➕ Folding rulers / tape measures | 4 | | R | Buy |
| ➕ Rulers 30 cm | 6 | | R | Buy |
| ➕ Pencils | 20 | | C | Buy |
| ➕ A4 card templates | 10 | Show the footprint while building | R | Build |
| Craft mats | 25 | About 60 × 40 cm, lie flat at the bottom of the box. Protect borrowed tables and benches; also the base for cutters and hot glue | R | Buy |

### 🔵 Blue 3: Power tool station (Eurobox 600 × 400 × 320)

Used at the **power tool station**, children with an adult volunteer next to them. See [Safety](../wiki/04-Safety-and-Nature.md#power-tool-station).

| Item | Qty | Spec / note | Type | Source |
|---|---|---|---|---|
| Cordless drill/driver | 2 | Light model, with clutch | R | Borrow / Sponsor |
| Drill bits and bit set | 1 set each | Wood 2–10 mm | R/C | Buy |
| Cordless jigsaw | 1 | With fine wood blades | R | Borrow / Sponsor |
| Jigsaw blades | 1 pack | | C | Buy |
| Batteries + chargers | 2 per tool, 1 charger each | Charged the day before | R | Borrow |
| Hot glue guns | 4 | **Low-temperature** models for kids | R | Buy |
| Hot glue sticks | 1 pack | Matching the guns' temperature | C | Buy |
| Heat-proof mats / silicone pads | 4 | Rest the guns here | R | Buy |
| Safe cardboard saw | 1 | Child-safe electric saw that cuts cardboard but not skin; own case | R | Buy / Sponsor |
| Multi-socket extension with switch | 1 | For glue guns, chargers and cardboard saw | R | Borrow |

### 🔴 Red: Safety and first aid (Eurobox 400 × 300 × 120)

| Item | Qty | Spec / note | Type | Source |
|---|---|---|---|---|
| First aid kit | 1 | e.g. DIN 13157 in Germany; check expiry dates | R/C | Buy |
| Plasters, tweezers, tick remover | 1 set | Extra | C | Buy |
| Burn gel / cooling pads | 1 set | For hot glue burns | C | Buy |
| Cut-resistant gloves | 4 pairs | For cutters and knives | R | Buy |
| Safety glasses | 15 | Child and adult sizes | R | Buy |
| Work gloves | 15 pairs | Child and adult sizes | R | Buy |
| Sunscreen | 2 | | C | Buy |
| Emergency sheet | 1 | Emergency number, address, access point, map | C | Build |
| Tool briefing picture cards | 1 set | Laminated | R | Build |

---

## 2. Material boxes

**Materials never cost money.** Everything here is donated, collected as litter or found in nature. Quantities are rough targets.

### 🟢 Green 1 + 2: Natural materials (2 × Eurobox 600 × 400 × 320)

| Item | Qty | Spec / note | Type | Source |
|---|---|---|---|---|
| Softwood offcuts | lots | Spruce, pine, poplar; sorted by size | C | Donate (carpenter, sawmill) |
| Wooden slats, strips, dowels | lots | Various diameters | C | Donate |
| Wine corks, natural | 300+ | | C | Donate (restaurants, households) |
| Bamboo sticks, reed | 30 | Only collect where allowed | C | Donate / Nature |
| Wooden skewers, toothpicks | 1 pack each | | C | Donate / Buy |
| Wooden clothes pegs | 50 | | R/C | Donate |
| Natural string (cotton, jute, hemp) | 5 rolls | | C | Donate / Buy |
| Cotton and linen fabric scraps | 1 bag | Sails | C | Donate |
| Beeswax | 2 blocks | Waterproofing | C | Donate (beekeeper) |

### 🟡 Yellow 1 + 2: Reused materials (2 × Eurobox 600 × 400 × 320)

| Item | Qty | Spec / note | Type | Source |
|---|---|---|---|---|
| Plastic bottles with caps | 30 | Rinsed, various sizes | C | Donate / Litter collection |
| Drink cartons | 30 | Rinsed and dried | C | Donate / Litter collection |
| Plastic containers (yoghurt pots etc.) | 20 | Clean | C | Donate |
| Old T-shirt fabric | 1 bag | | C | Donate |
| Tin lids | 10 | Edges folded | C | Donate |

### ⚪ Grey 1: Connectors (Eurobox 400 × 300 × 120)

| Item | Qty | Spec / note | Type | Source |
|---|---|---|---|---|
| Rubber bands, natural rubber | 200 | Count out and back in | C | Buy / Donate |
| Screws, assorted | 1 box | In a divider box | C | Donate |
| Nails, small | 1 box | | C | Donate |
| Wire, soft | 1 roll | | C | Donate |
| Wooden dowels 6 and 8 mm | 50 each | | C | Buy / Donate |

### ⚪ Grey 2: Consumables (Eurobox 400 × 300 × 120)

| Item | Qty | Spec / note | Type | Source |
|---|---|---|---|---|
| Sandpaper | 10 each | Grits 80, 120, 180 | C | Buy |
| Wood glue D3 | 3 small bottles | | C | Buy |
| Water-based paints + brushes | 1 set | | C | Buy / Donate |
| Masking tape | 2 | For marking only, not for boats | C | Buy |

### Material collection (before the event)

| Item | Qty | Spec / note | Type | Source |
|---|---|---|---|---|
| Collection boxes | 3–5 | Cardboard or crates, decorated, with traffic light label | R | Build |
| Litter pickers | 10 | For the litter collection walk | R | Borrow (municipality, clean-up day) |
| Gloves for litter collection | 20 pairs | Child sizes too | R/C | Borrow / Buy |
| Bags or buckets | 10 | For sorting what's found | R | Borrow |

---

## 3. Race office boxes

The race office needs more than one box. Split it into **office** and **gallery**.

### ⚫ Black 1: Race office (Eurobox 600 × 400 × 220)

| Item | Qty | Spec / note | Type | Source |
|---|---|---|---|---|
| Boat register, kids' list | 5 sheets | Clusters of three, see [template](../templates/boat-register.md) | C | Print |
| Boat register, grown-ups' list | 2 sheets | | C | Print |
| Cloakroom tickets, colour A | 1 roll | Kids, nos. 1–499, stub = ice cream voucher | C | Buy |
| Cloakroom tickets, colour B | 1 roll | Grown-ups, from no. 500, stubs removed | C | Buy |
| Extra ice cream vouchers | 40 | Nos. from 501, for children without their own boat | C | Print |
| Envelope for removed stubs | 1 | | C | |
| Ice cream stamp + ink pad | 1 | Optional, back-of-hand stamp | R | Buy |
| Prize catalogue, sponsor sheets | 1 set | | C | Print |
| Jury score sheets | 5 | | C | Print |
| Photo consent forms | 50 | | C | Print |
| Certificates | boats + 20% | Thick recycled paper | C | Print |
| Clipboards | 6 | | R | Buy / Borrow |
| Pens | 10 | | C | Buy |
| Stopwatches | 3 | Or phones | R | Buy / Own |
| Kitchen scale | 1 | 1 g steps, up to 5 kg | R | Buy / Borrow |
| Ruler 50 cm | 1 | For boat height | R | Buy |
| Megaphone + batteries | 1 | **Backup** for the sound system, charged | R | Borrow (fire brigade) |
| "No photo" stickers or wristbands | 20 | One colour | C | Buy |
| Cash box + receipt book | 1 | Grown-up start fees, donations | R/C | Buy |
| Kit logbook | 1 | | R | |

### ⚫ Black 2: Gallery, vote and photo station (Eurobox 600 × 400 × 320)

| Item | Qty | Spec / note | Type | Source |
|---|---|---|---|---|
| Gold talers | participants + 20% | Wooden discs 3–4 cm, water-based gold paint | R | Build |
| Silver talers | spectators + 20% | Same, silver | R | Build |
| Voting containers | 60 | Stackable tins, jars or small boxes | R | Donate / Buy |
| Number cards for containers | 60 | Large numbers, laminated, swappable | R | Build |
| A4 grid mat | 1 | A4 frame with 1 cm grid | R | Build |
| Neutral photo backdrop | 1 | Light fabric or board | R | Donate |
| Small tripod / phone holder | 1 | | R | Buy / Own |
| White umbrella | 1 | Soft light, shade | R | Borrow |
| Prize bags | number of prizes | Paper or cloth | C/R | Buy / Sponsor |
| Rubber ducks, numbered | 100 | Optional duck race, reused every year | R | Buy |

### Check station (next to the race office)

| Item | Qty | Spec / note | Type | Source |
|---|---|---|---|---|
| A4 gauges | 2–3 | Base board with inner frame 210 × 297 mm | R | Build |
| Measuring post | 1 | Vertical batten with cm scale | R | Build |
| Materials traffic light poster | 1 | | R | Print |

---

## 4. Infrastructure

| Item | Qty | Spec / note | Type | Source |
|---|---|---|---|---|
| **Tents / pavilions** | | | | |
| Building stations | 2 | 3 × 6 m, or 4 × 3 × 3 m | R | Borrow (fire brigade, club) |
| Race office + check + photo station | 1 | 3 × 6 m | R | Borrow |
| Gallery | 1 | 3 × 6 m; protects boats from sun and rain | R | Borrow |
| First aid | 1 | 3 × 3 m, or a corner of the race office tent | R | Borrow |
| Tent weights | 4 per tent | Water canisters or concrete weights; pegs don't hold everywhere | R | Borrow |
| **Tables and benches** | | | | |
| Beer tent sets (table + 2 benches) | 9 | Building stations (5), power tool station (1), spectators and breaks (3). **Cover building tables and benches with craft mats** (see Blue 2) | R | Borrow |
| Extra tables | 12 | Gallery (7–8 tables for 50 boats), race office (2), photo station (1), info stand (1) | R | Borrow |
| **Water and hygiene** | | | | |
| Hand-washing station | 1–2 | 20 l canister with tap, bucket, soap, paper towels | R/C | Build / Borrow |
| Toilets | 1–2 | Nearby building, or rented with hand-wash | – | Borrow / Rent |
| Drinking water | 1 station | Canisters or tap nearby | R | Borrow |
| Waste bins, sorted | 3 | Paper, packaging, residual waste; plus a "back into the material box" crate | R | Borrow |
| **Power and sound** | | | | |
| **Mobile power station** (portable battery) | 1 | Always with us. Powers the sound system, the power tool station (glue guns, cardboard saw, chargers) and phone charging. Quiet and independent of an outlet. Charge it fully the day before | R | Buy / Sponsor |
| Sound system with microphone | 1 | For the commentator: announcing boats, heats and prizes. Wireless mic is handy along the course | R | Buy / Borrow |
| Extension cable reel / multi-sockets | 1–2 | Outdoor rated, fully unrolled, cables secured against tripping | R | Borrow |
| Backup power | – | Outlet nearby (village hall, fire station) in case the battery runs low | – | Borrow |
| **Site** | | | | |
| Spectator zone marking | 100 m | Reusable rope with pennants (bunting) instead of barrier tape | R | Borrow / Build |
| Directional signs | 5 | Parking, toilets, registration, first aid, gallery | R | Build |
| Site map poster | 2 | At race office and entrance | C | Print |
| "Rules at a glance" poster | 3 | Building stations, race office, start | C | Print |
| Award stage | 1 | Pallets with a board, a trailer or just a marked spot | R | Borrow |
| Info stand | 1 | Table with silver talers, questions, lost children | R | Borrow |
| Rain alternative | 1 | Barn, hall or fire station nearby | – | Borrow |
| **External** | | | | |
| Ice cream vendor | 1 | On site or within walking distance; fixed price per voucher | – | Agreement |

---

## 5. Race course

| Item | Qty | Spec / note | Type | Source |
|---|---|---|---|---|
| **Start** | | | | |
| Start plank | 1 | Untreated spruce or larch, 30–40 mm thick, 20–25 cm wide, creek width + 1 m long | R | Donate (sawmill) / Buy |
| Gate batten | 1 | Thin board, length = creek width | R | Donate |
| Hinges or cord loops | 3 | | R | Buy |
| Pull cord | 5 m | | R | Buy |
| **Finish** | | | | |
| Finish plank | 1 | Same as start plank | R | Donate / Buy |
| Finish line paint | 1 small tin | Water-based, white | C | Buy |
| Flag line | 1 | Light string with small flags above the water | R | Build |
| **Fixing** | | | | |
| Wooden stakes | 20 | 4 per board, 4 for the catch net, rest for chicanes and spare | R | Buy / Build |
| Mallet | 1 | | R | Borrow |
| Lashing straps | 4 | Hold planks to stakes | R | Borrow |
| **Catch and rescue** | | | | |
| Catch net | 1 | Fine net, creek width + 2 m (old football goal net, fruit tree net) | R | Borrow (sports club) |
| Landing net | 1 | Backup below the catch net | R | Borrow |
| Raven sticks | 4 | 1.5–2 m, soft tip, light enough for children; builders use them from the bank | R | Build |
| Throw lines / rescue poles | 2 | Start and finish | R | Borrow (fire brigade, DLRG) |
| Waders or rubber boots | 2–4 pairs | For course helpers | R | Own / Borrow |
| High-visibility vests | 10 | Printed "Rabenregatta team" if possible | R | Buy / Sponsor |
| Water level marker stake | 1 | Mark of the maximum water level for racing | R | Build |
| **Testing** | | | | |
| Test harbour | 1–2 | Big tub, mortar trough or paddling pool | R | Borrow |
| Buckets | 2 | To fill the test harbour | R | Borrow |
| Mini creek (optional) | 1 | 2–3 m rain gutter or V-channel, garden hose | R | Donate (roofer) / Build |
| **Chicanes** | | | | |
| Chicanes | 1 per sponsor | Built and brought by the sponsors; natural stone, clinker, untreated wood preferred | R | Sponsor |
| Chicane signs | 1 per sponsor | Small wooden sign: "Leash curtain, by …" | R | Build |
| **Spare parts bag** | 1 | Screws, cord, spare hinge, cable ties | C | |

---

## 6. Marketing, banner and sponsors

| Item | Qty | Spec / note | Type | Source |
|---|---|---|---|---|
| **Event branding** | | | | |
| Event banner | 1 | e.g. 3 × 1 m fabric, **without a date** so it can be reused; add the date on a swappable board | R | Buy / Sponsor |
| Date board for the banner | 1 | Chalkboard or swappable sign | R | Build |
| Raven logo | 1 | SVG, for all materials | R | Design |
| **Print** | | | | |
| Posters A3 | 20 | [Poster text](../marketing/poster-text.md), recycled paper; with all prizes and win conditions | C | Print |
| Posters A4 | 30 | Shops, bus stops, notice boards | C | Print |
| Flyers A5 | 200 | Schools, kindergartens, clubs | C | Print |
| Collection box posters | 5 | Version C of the poster text | C | Print |
| Letters to schools | as needed | [Template](../marketing/letter-to-schools.md) | C | Print |
| Letters to businesses | as needed | [Template](../marketing/letter-to-businesses.md) | C | Print |
| **Sponsors** | | | | |
| Sponsor sheets | 1 per sponsor | [Template](../templates/sponsor-sheet.md) | C | Print |
| Sponsor board at the entrance | 1 | All sponsors, same size, with their chicane and prize | R | Build |
| Chicane signs | see race course | | | |
| Sponsor prizes | 1 per sponsor | Paid or provided by the sponsor | – | Sponsor |
| Extras for every prize bag | per sponsor offer | e.g. €20 book voucher from the dog school | – | Sponsor |
| **Prizes and certificates** | | | | |
| Prizes | 2 fixed + 1 per sponsor | Vouchers, books, wooden medals, plants; no plastic trophies | – | Sponsor / Donate |
| Certificates | see race office | | | |
| **Documentation and fan culture** | | | | |
| Camera or good phone | 1–2 | Photo station and chicanes | R | Own |
| Boat trading cards | optional | One per boat, printed after the event | C | Print |
| "Boat of the year" poster | optional | Town hall, school | C | Print |
| **Digital** (no cost) | | Press release, social posts, online gallery, QR code to the event page | – | [Templates](../marketing/) |

---

## Summary: the box kit

| Box | Size | Contents |
|---|---|---|
| 🔵 Blue 1 | 600 × 400 × 220 | Cutting |
| 🔵 Blue 2 | 600 × 400 × 220 | Assembling and measuring |
| 🔵 Blue 3 | 600 × 400 × 320 | Power tool station |
| 🟢 Green 1 + 2 | 2 × 600 × 400 × 320 | Natural materials |
| 🟡 Yellow 1 + 2 | 2 × 600 × 400 × 320 | Reused materials |
| ⚪ Grey 1 | 400 × 300 × 120 | Connectors |
| ⚪ Grey 2 | 400 × 300 × 120 | Consumables |
| 🔴 Red | 400 × 300 × 120 | Safety and first aid |
| ⚫ Black 1 | 600 × 400 × 220 | Race office |
| ⚫ Black 2 | 600 × 400 × 320 | Gallery, vote and photo station |

**12 boxes.** Plus the items that don't fit in boxes: safe cardboard saw (own case), planks, stakes, raven sticks, catch net, A4 gauges, test harbour, banner.

## What to buy first

With a small budget, prioritise like this:

1. **Safety:** first aid kit, safety glasses, gloves
2. **Core hand tools:** hand saws, scissors, hammers, pliers, garden pruners, sanding sponges
3. **Power tool station:** hot glue guns first (cheap and most used), then borrow drill and jigsaw
4. **Mobile power station and sound system:** if you don't have one yet, ask a sponsor
5. **Race course:** planks, stakes, cord (catch net borrowed)
6. **Race office:** cloakroom tickets, kitchen scale, stopwatches (phones work too)
7. **Euroboxes:** start with what you have; buy boxes over time
8. **Banner:** once there's a sponsor for it

Everything else: borrow, build or ask a sponsor. See the [budget template](../templates/budget.md).
