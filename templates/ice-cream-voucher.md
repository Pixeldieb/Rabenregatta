# Ice Cream Voucher

*Main method: the **tear-off stub of the cloakroom ticket** that is the boat's start number. See [Registration and race office](../wiki/17-Race-Office.md#cloakroom-tickets-start-number-and-ice-cream-in-one).*

*Rule: **one ice cream per child**, no matter how many boats it registers. For a child's further boats, the race office removes the stub before handing out the ticket.*

*This template is for **extra vouchers**, for children without their own boat (e.g. siblings who built a boat together). Print on card, 8 per A4 page, numbered from 501 upwards so they don't overlap with the ticket numbers. Stamp each one on the back. See [Money and fairness](../wiki/15-Money-and-Fairness.md#ice-cream-vouchers).*

---

| | |
|---|---|
| 🍦 **Rabenregatta _[village, year]_** | 🍦 **Rabenregatta _[village, year]_** |
| **One ice cream** for a brave boat builder! | **One ice cream** for a brave boat builder! |
| Redeem at: _[ice cream vendor]_ | Redeem at: _[ice cream vendor]_ |
| Valid: _[date]_ | Valid: _[date]_ |
| No. **501** | No. **502** |

*(repeat)*

---

## Agreement with the ice cream vendor

| | |
|---|---|
| Vendor | |
| Contact | |
| Price per voucher | € |
| What the voucher covers | e.g. one scoop in a cone or cup |
| Valid | ☐ event day only ☐ until _[date]_ |
| Settlement | Vendor hands in all vouchers by _[date]_; counted together; paid against an invoice within _[14]_ days |

## Voucher log

| Number range | Handed out by | To (group) | Date |
|---|---|---|---|
| Tickets 1–60 | Race office | Builders (stubs of the cloakroom tickets) | |
| 501–540 | Race office | Children without their own boat | |
| | | | |

**Handed out:** ______ **Redeemed (counted with the vendor):** ______ **Amount paid:** € ______
